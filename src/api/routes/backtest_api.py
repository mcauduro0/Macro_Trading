...
def _sample_portfolio_result(strategy_ids: list[str], weights: dict[str, float]) -> dict:
...
async def run_backtest(request: BacktestRunRequest):
    ...
    try:
        ...
        if request.strategy_id not in ALL_STRATEGIES and request.strategy_id not in StrategyRegistry._strategies:
            raise HTTPException(
                status_code=404,
        ...
        # Determine dates
        start = date.fromisoformat(request.start_date) if request.start_date else date(2020, 1, 1)
        ...
        if hasattr(result, "equity_curve") and result.equity_curve:
            equity_curve = [
                float(e) for _, e in result.equity_curve
            ]

        return _envelope({
            "strategy_id": request.strategy_id,
            "sharpe_ratio": float(result.sharpe_ratio) if hasattr(result, "sharpe_ratio") else None,
            "annual_return": float(result.annualized_return) if hasattr(result, "annualized_return") else None,
...
async def get_backtest_results(
    ...
    try:
        ...
        logger.debug("backtest_results_db_unavailable strategy_id=%s error=%s", strategy_id, exc)
