...
<task type="auto">
  <name>Task 2: Implement FX-04 Vol Surface RV, FX-05 Terms of Trade, and tests</name>
  <files>
    src/strategies/fx_04_vol_surface_rv.py
    src/strategies/fx_05_terms_of_trade.py
    tests/test_strategies/test_fx_new.py
...
" && python -m pytest tests/test_strategies/test_fx_new.py -v --tb=short 2>&1 | tail -20
  ...
  <done>All 4 FX strategies (FX-02 through FX-05) registered in StrategyRegistry, all tests pass confirming signal generation with correct z_score, entry_level, stop_loss, take_profit fields</done>
</task>

</tasks>

<verification>
1. `python -c "from src.strategies.registry import StrategyRegistry; print(StrategyRegistry.list_all())"` shows FX_02, FX_03, FX_04, FX_05 alongside existing strategies
...
3. `python -m pytest tests/test_strategies/test_fx_new.py -v` — all tests pass
4. Each strategy file contains `@StrategyRegistry.register(...)` decorator and `StrategySignal` output
5. Backtest smoke-test: `python -c "from src.strategies.fx_02_carry_momentum import Fx02CarryMomentumStrategy; from src.strategies.fx_03_flow_tactical import Fx03FlowTacticalStrategy; from src.strategies.fx_04_vol_surface_rv import Fx04VolSurfaceRvStrategy; from src.strategies.fx_05_terms_of_trade import Fx05TermsOfTradeStrategy; from src.backtesting.engine import BacktestEngine; from src.backtesting.costs import TransactionCostModel; print('All 4 FX strategies instantiate and are compatible with BacktestEngine and TransactionCostModel')"` — confirms v2 framework compatibility
</verification>
