...
<task type="auto">
  <name>Task 2: Implement CROSS-01 Regime Allocation, CROSS-02 Risk Appetite, tests, and consolidate all 16 new strategy imports into __init__.py</name>
  <files>
    src/strategies/cross_01_regime_allocation.py
    src/strategies/cross_02_risk_appetite.py
    src/strategies/__init__.py
...
**Update `src/strategies/__init__.py`** (consolidates ALL 16 new strategies from Plans 01-04):
- Import all 16 new strategy classes from all 4 plans:
  - From Plan 01 (FX): FX_02, FX_03, FX_04, FX_05
  - From Plan 02 (Rates): RATES_03, RATES_04, RATES_05, RATES_06
  - From Plan 03 (Inflation/Cupom): INF_02, INF_03, CUPOM_02
  - From Plan 04 (Sovereign/Cross): SOV_01, SOV_02, SOV_03, CROSS_01, CROSS_02
- Add all 16 to ALL_STRATEGIES dict
- Add all 16 to __all__ list
- At this point ALL_STRATEGIES should have 24 entries (8 existing + 16 new)
- This is done in Plan 04 (Wave 2) specifically because it depends on Plans 01-03 (Wave 1) completing first, avoiding parallel write conflicts to __init__.py
...
7. Backtest smoke-test: `python -c "from src.strategies.sov_01_cds_curve import Sov01CdsCurveStrategy; from src.strategies.sov_02_em_relative_value import Sov02EmRelativeValueStrategy; from src.strategies.sov_03_rating_migration import Sov03RatingMigrationStrategy; from src.strategies.cross_01_regime_allocation import Cross01RegimeAllocationStrategy; from src.strategies.cross_02_risk_appetite import Cross02RiskAppetiteStrategy; from src.backtesting.engine import BacktestEngine; from src.backtesting.costs import TransactionCostModel; print('All 5 SOV/CROSS strategies compatible with BacktestEngine and TransactionCostModel')"` — confirms v2 framework compatibility
</verification>
...
- All tests pass (>=17)
- __init__.py updated with all 16 new strategies (consolidated from Plans 01-04 in this Wave 2 plan)
</success_criteria>
