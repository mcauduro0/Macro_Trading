...
must_haves:
  truths:
    ...
    - "Two scheduled runs: EOD (after market close) and pre-open (06:30 BRT, offset from daily_pipeline)"
...
Purpose: Automate the daily PMS workflow so the manager sees fresh data (MTM, proposals, morning pack, attribution) when they log in each morning. Two schedules: EOD run (market close, ~18:00 BRT / 21:00 UTC) for MTM + attribution, and pre-open run (06:30 BRT / 09:30 UTC) for morning pack + proposals. Pre-open is offset 30 min from existing daily_pipeline_schedule (09:00 UTC) to avoid contention. Each step warms the Redis cache after DB persistence per locked decision.
...
- Implementation:
  ...
  2. Call `morning_pack.generate(briefing_date=date.today())` (explicit required argument per MorningPackService API)
...
- Implementation:
  ...
  2. Call `attribution.compute_for_period("daily")` (actual method name per PerformanceAttributionEngine API)
...
**Update `all_assets`**: Extend with all 4 PMS assets (add comment `# PMS (4)` and append pms_mark_to_market, pms_trade_proposals, pms_morning_pack, pms_performance_attribution).

**EOD Job** (after market close -- per locked decision: MTM + attribution):
```python
...
**Pre-Open Job** (morning -- per locked decision: morning pack + proposals; MTM included as upstream dep only):
```python
...
# Pre-open: 09:30 UTC = 06:30 BRT, weekdays
...
Run: `python -c "from src.orchestration.definitions import defs; print(f'Assets: {len(defs.get_all_asset_keys())}, Jobs: {len(list(defs.get_all_job_defs()))}, Schedules: {len(list(defs.get_all_schedule_defs()))}')"` -- should show 26 assets, 4 jobs, 3 schedules.
  ...
  <done>definitions.py updated with 26 total assets (22 existing + 4 PMS), 4 jobs (daily_pipeline, bronze_ingest, pms_eod, pms_preopen), 3 schedules (daily, EOD, pre-open). Pre-open job selects 3 assets (MTM + proposals + morning_pack); attribution is EOD-only per locked decision.</done>
</task>
...
- Two scheduled jobs: EOD (18:00 BRT / 21:00 UTC) and pre-open (06:30 BRT / 09:30 UTC, offset from daily_pipeline)
