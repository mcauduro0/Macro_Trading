...
must_haves:
  truths:
    ...
    - "PerformanceAttributionEngine decomposes P&L by strategy, asset class, instrument, factor, time period (weekly/monthly sub-buckets), and trade type"
...
8. **by_time_period**: Sub-period breakdown within the selected date range. Group daily P&L snapshots from _pnl_history into calendar buckets: weekly buckets if range <= 90 days, monthly buckets if range > 90 days. Each bucket: `{period_start, period_end, label (e.g., "Week 1", "Jan 2026"), pnl_brl, return_pct, cumulative_pnl_brl}`. Sum must equal total.

9. **by_trade_type**: `{systematic: {pnl_brl, count}, discretionary: {pnl_brl, count}}`. Discretionary = signal_source == "DISCRETIONARY" in linked proposals (check trade_workflow if available), else all systematic.

10. **performance_stats**: Compute from daily P&L snapshots in _pnl_history within period: annualized_return_pct, annualized_vol_pct, sharpe_ratio (rf=0 for simplicity), sortino_ratio, max_drawdown_pct, win_rate_pct (% positive days), profit_factor (sum gains / abs sum losses), avg_win_brl, avg_loss_brl.
...
- PerformanceAttributionEngine.compute_attribution() returns dict with 10 decomposition groups (including by_time_period)
