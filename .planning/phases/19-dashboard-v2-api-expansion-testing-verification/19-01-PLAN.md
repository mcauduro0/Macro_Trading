...
- Call `useWebSocket('/ws/alerts')` to get `{connected, lastMessage}`. Use React.useEffect watching `lastMessage` to push new alerts into a `toasts` state array (React.useState). Each toast object: `{id, message, timestamp}`. Auto-dismiss toasts after 10 seconds via setTimeout removing by id.
- Derive `alertCount` from `toasts.length` (unread/active alerts). Pass `alertCount={alertCount}` to `<Sidebar />`.
- Add a ToastContainer component at the bottom-right that renders the `toasts` array as stacked notification cards (fixed bottom-right, z-50). Each toast shows message text and a close button.
