...
- Abstract method: `generate_signals(self, as_of_date: date) -> list[StrategyPosition]` — subclasses implement to produce target positions. **Timing convention (per locked decision):** signals generated for date D use data available at D's bar close and are executed at next open (D+1). The backtesting engine (Phase 10) enforces this via PIT data loader; strategies only need to use `as_of_date` consistently
