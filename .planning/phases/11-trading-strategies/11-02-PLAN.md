...
6. Strategy logic (mean reversion on spread overshoot after large UST move):
   - If abs(weekly_ust_change) * 10000 < ust_weekly_move_bps: no signal (UST did not move enough)
   - If spread z-score > spread_z_threshold (DI-UST spread too wide — DI rates overshot the UST move):
     - LONG DI position (expect DI rates to fall back toward equilibrium spread = bond price rises)
   - If spread z-score < -spread_z_threshold (DI-UST spread too narrow — DI rates underreacted to UST move):
     - SHORT DI position (expect DI rates to rise toward equilibrium spread = bond price falls)
