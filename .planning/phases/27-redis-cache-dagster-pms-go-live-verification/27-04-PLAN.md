...
17. `verify_risk_monitor`: Import RiskMonitorService, check get_snapshot method
18. `verify_attribution`: Import PerformanceAttributionEngine, check compute method
...
20. `verify_pms_api_routes`: Import app from src.api.main, check 6 PMS routers registered (pms_portfolio, pms_trades, pms_journal, pms_briefing, pms_risk, pms_attribution). Count PMS routes >= 20.
...
morning-pack:
	python -c "from src.pms.morning_pack import MorningPackService; import asyncio; ms = MorningPackService(); print(asyncio.run(ms.generate()) if hasattr(ms, 'generate') else 'MorningPackService ready')"
