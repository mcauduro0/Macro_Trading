...
Output: `src/cache/pms_cache.py` with PMSCache class, integrated into 5 PMS API route files with cache-first reads and write-through invalidation. (pms_journal.py excluded -- journal entries are an append-only audit log with diverse query patterns, not a compute-heavy endpoint requiring caching.)
</objective>
...
- All 5 cacheable PMS route files integrated with cache (pms_journal.py excluded as audit log)
