...
All asset functions follow the same pattern established in 18-01: context.log logging, status dict return on success, **re-raise exceptions on failure** so Dagster RetryPolicy handles retries natively. Do NOT catch and swallow exceptions. Each function body should be 15-30 lines wrapping the existing business logic modules.
  </action>
  <verify>
