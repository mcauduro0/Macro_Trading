...
2. **Enhance src/backtesting/engine.py** (per BTST-01 and BTST-02): Add run_portfolio and walk_forward_validation to BacktestEngine. Keep existing run() method untouched.
   ...
   - `run_portfolio(strategies: list, weights: dict[str, float] | None = None) -> dict`:
     ...
     e. Return dict with keys: "portfolio_result" (BacktestResult for combined), "individual_results" (dict of strategy_id -> BacktestResult), "weights" (the weights used), "correlation_matrix" (dict of {(id_a, id_b): corr}), "attribution" (dict of strategy_id -> contribution_pct)
