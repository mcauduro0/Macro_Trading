...
must_haves:
  truths:
    - "RiskLimitChecker has 9 configurable risk limits (max VaR 95%, max VaR 99%, max drawdown, max leverage, max single position, max asset class concentration, max risk budget, max strategy daily loss, max asset class daily loss)"
    ...
    - "Circuit breaker events are logged with full context (positions, P&L, signal state) AND dispatched via configurable webhook/email alert"
...
**AlertDispatcher (within drawdown_manager.py — implements CONTEXT.md locked decision: "send real-time alert (webhook/email)"):**

9. Define `AlertDispatcher` class:
   - `__init__(self, webhook_url: str | None = None, enabled: bool = True)`:
     - `self.webhook_url` — optional HTTP endpoint for alert delivery
     - `self.enabled` — toggle to disable alerting in tests or dry-run
     - Uses structlog for all log output
   - `dispatch(self, event: CircuitBreakerEvent) -> bool`:
     - **Always**: Log the event at WARNING level via structlog with full context (state_from, state_to, drawdown_pct, action, timestamp)
     - **If webhook_url is set and enabled**: POST JSON payload to webhook_url using `urllib.request.urlopen` (stdlib, no new dependency). Payload: `{"event_type": "circuit_breaker", "state_from": str, "state_to": str, "drawdown_pct": float, "action": str, "timestamp": str, "positions_snapshot": dict, "pnl_at_trigger": float}`. Catch `URLError`/`HTTPError` and log failure at ERROR level — never raise (alerting must not crash the trading loop).
     - Return True if dispatch succeeded (or webhook not configured), False if HTTP POST failed.

10. Wire AlertDispatcher into DrawdownManager:
    - `DrawdownManager.__init__` accepts optional `alert_dispatcher: AlertDispatcher | None = None`
    - On every state transition in `update()`, after appending to event_log, call `self.alert_dispatcher.dispatch(event)` if dispatcher is set
    - Similarly wire into `StrategyLossTracker` and `AssetClassLossTracker` — accept optional dispatcher, call on breach events
...
**Update __init__.py:**
Add exports: RiskLimitsConfig, RiskLimitChecker, LimitCheckResult, CircuitBreakerState, CircuitBreakerConfig, CircuitBreakerEvent, DrawdownManager, AlertDispatcher, StrategyLossTracker, AssetClassLossTracker, RiskMonitor, RiskReport.
  </action>
  <verify>
  ...
  <done>
    ...
    - Circuit breaker events logged with full context (positions, P&L, signals) and dispatched via AlertDispatcher (structlog + optional webhook POST)
    - Per-strategy and per-asset-class loss trackers fire independently
    - AlertDispatcher handles webhook failures gracefully (no crash on HTTP error)
...
- test_alert_dispatcher_logs_on_transition: AlertDispatcher.dispatch() logs event at WARNING level via structlog (mock structlog, verify call)
- test_alert_dispatcher_webhook_post: AlertDispatcher with webhook_url sends HTTP POST on dispatch (mock urllib.request.urlopen, verify JSON payload)
- test_alert_dispatcher_webhook_failure_no_crash: AlertDispatcher handles HTTP error gracefully (mock urlopen to raise URLError, verify returns False, no exception raised)
- test_drawdown_manager_calls_dispatcher: DrawdownManager with AlertDispatcher calls dispatch() on state transition (mock dispatcher, trigger L1, verify dispatch called with event)
...
Run: `python -m pytest tests/test_risk/ -v`
  ...
  <verify>
    ...
    - Total test count: ~11 (VaR) + ~10 (stress) + ~11 (limits) + ~20 (drawdown incl. alert dispatcher) + ~8 (monitor) = ~60 tests
  ...
  <done>
    ...
    - DrawdownManager: 20 tests covering all state transitions, cooldown, recovery, event logging, alert dispatch (structlog + webhook), and per-strategy/per-asset-class trackers
    - RiskMonitor: 8 tests covering report generation, risk level classification, and text formatting
    - Total TESTV2-04 coverage: ~60 tests across all risk management components
...
<verification>
1. `python -m pytest tests/test_risk/ -v` — all ~60 tests pass
2. `python -m pytest tests/test_portfolio/ tests/test_risk/ -v` — all Phase 12 tests pass together (~88 total)
...
- All TESTV2-04 tests pass (~60 tests for risk management alone)
- Total Phase 12 test count: ~88 tests across portfolio + risk modules
</success_criteria>
