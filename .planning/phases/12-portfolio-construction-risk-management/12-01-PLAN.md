...
4. `SignalAggregator.aggregate(self, agent_reports: dict[str, AgentReport]) -> list[AggregatedSignal]`:
   - For each asset class in AssetClass (FIXED_INCOME, FX, EQUITY_INDEX, COMMODITY):
     ...
     - **CrossAsset veto check**: If cross_asset_agent's CROSSASSET_REGIME signal value is extreme (< -0.7 implies extreme risk-on which is unusual to veto; > 0.7 implies extreme risk-off), apply veto: reduce net_score by 50% and set veto_applied=True. The veto fires when regime_score > 0.7 (extreme risk-off) to protect capital.
...
Use structlog for logging throughout. No database access — these are pure computation modules.
  ...
  <done>
    ...
    - CrossAsset veto fires when regime_score > 0.7 and reduces positions by 50%
...
Run: `python -m pytest tests/test_portfolio/ -v`
  ...
  <done>
    ...
    - All unit tests pass: SignalAggregator (~9), PortfolioConstructor (~9), CapitalAllocator (~9) = ~27 total tests
...
<verification>
1. `python -m pytest tests/test_portfolio/ -v` — all ~27 tests pass
...
- ~27 unit tests pass, zero lint errors
</success_criteria>
