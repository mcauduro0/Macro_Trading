...
must_haves:
  truths:
    ...
    - "RATES-06 generates StrategySignal positioning around COPOM [-5,+2] days based on DI-implied Selic vs Focus median divergence, with adaptive post-event exit"
  ...
  key_links:
    ...
    - from: "src/strategies/rates_06_copom_event.py"
      to: "PointInTimeDataLoader"
      via: "get_curve for DI, get_latest_macro_value for Selic/Focus"
...
<task type="auto">
  <name>Task 2: Tests for all 4 new rates strategies and update __init__.py</name>
  <files>
    tests/test_strategies/test_rates_new.py
...
- Import all 4 new rates strategy classes
- Add to ALL_STRATEGIES dict: "RATES_03", "RATES_04", "RATES_05", "RATES_06"
- Add to __all__ list
...
- At least 18 tests total
  ...
  <done>All 4 rates strategies imported in __init__.py, listed in ALL_STRATEGIES, and >=18 tests pass confirming signal generation, event windowing, adaptive exit, and missing-data handling</done>
</task>
